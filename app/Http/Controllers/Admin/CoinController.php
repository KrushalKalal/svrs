...
class CoinController extends Controller
{
    ...
    public function coin_store(Request $request)
    {
        ...
        return response()->json(['status' => true, 'message' => 'Coin Created Successfully']);
    }
    ...
    public function coin_update(Request $request)
    {
        ...
        return response()->json(['status' => true, 'message' => 'Coin Updated Successfully']);
    }
    ...
    public function coin_chart(Request $request)
    {
        ...
        if ($type == 'today') {
            $charts = CoinChart::whereDate('created_at', Carbon::today())->get();
            if ($charts->isEmpty()) {
                $charts = CoinChart::whereDate('created_at', Carbon::yesterday())->get();
            }
            if ($charts->isEmpty()) {
                $charts = CoinChart::orderBy('created_at', 'DESC')->take(1)->get();
            }
        } elseif ($type == 'week') {
            $charts = CoinChart::where('created_at', '>=', Carbon::now()->subDays(7))
            ...
            $charts = CoinChart::orderBy('created_at', 'ASC')->get();
        } else {
    ...
    public function coin_trade(Request $request)
    {
        ...
        $coin = Coin::first();
        if (!$coin) {
            return response()->json(['status' => false, 'message' => 'Coin not configured by admin']);
        }
        ...
        if ($totalAmount <= 0) {
            return response()->json(['status' => false, 'message' => 'Invalid trade amount']);
        }
        ...
        if ($request->type === 'sell') {
            $totalBuy = CoinTrade::where('user_id', $userId)->where('coin_id', $coin->id)->whereIn('type', ['buy', 'reward'])->sum('quantity');
            $totalSell = CoinTrade::where('user_id', $userId)->where('coin_id', $coin->id)->where('type', 'sell')->sum('quantity');
            $available = $totalBuy - $totalSell;

            if ($request->quantity > $available) {
                return response()->json([
                    'status' => false,
                    'message' => 'Not enough coin balance. Available: ' . number_format($available, 4)
                ]);
            }
        ...
        // PHASE 2: Fire referral rewards on every buy (top-up)
        if ($request->type === 'buy') {
            $member = auth()->user();
            if ($member->is_refer_member == 1 && $member->status == 1) {
                $this->rewardService->processRewards($member, $request->quantity);
            }
        }
