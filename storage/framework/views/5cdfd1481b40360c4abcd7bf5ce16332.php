<?php $__env->startSection('title', 'SVRS Coin'); ?>
<?php $__env->startSection('nav-title', 'SVRS Coin'); ?>
...
<?php $__env->startSection('content'); ?>
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;">
                <div>
                    ...
                    <p style="font-size:28px;font-weight:800;color:var(--gold);" id="livePriceDisplay">
                        ₹<?php echo e(number_format($currentPrice ?? 0, 2)); ?>
                ...
                <div style="text-align:right;">
                    ...
                    <p style="font-size:18px;font-weight:800;"><?php echo e(number_format($coinBalance ?? 0, 4)); ?> SVRS</p>
                    <p style="font-size:12px;color:var(--muted);">≈
                        ₹<?php echo e(number_format(($coinBalance ?? 0) * ($currentPrice ?? 0), 2)); ?></p>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="app-card app-card-inner">
                <div class="info-row" style="padding:0 0 12px;border-bottom:1px solid var(--border2);margin-bottom:14px;">
                    ...
                    <span class="val" style="color:var(--accent);">₹<span
                            id="walletBal"><?php echo e(number_format($wallet->balance ?? 0, 2)); ?></span></span>
                </div>
    ...
    <div id="panelOverview" style="display:none;">
        ...
        <div style="margin:0 20px;" class="app-card">
            <div class="info-row px" style="padding-top:14px;">
                ...
                <span class="val"><?php echo e(number_format($coinBalance ?? 0, 4)); ?> SVRS</span>
            </div>
            <div class="info-row px">
                <span class="key">Approx Value</span>
                <span class="val"
                    style="color:var(--green);">₹<?php echo e(number_format(($coinBalance ?? 0) * ($currentPrice ?? 0), 2)); ?></span>
            </div>
            <div class="info-row px" style="padding-bottom:14px;">
                <span class="key">Current Price</span>
                <span class="val" style="color:var(--gold);">₹<?php echo e(number_format($currentPrice ?? 0, 4)); ?></span>
            </div>
        </div>
        <div style="height:8px;"></div>
    </div>
    ...
    <div id="panelHistory" style="display:none;">
        <div style="padding:16px 20px 0;">
            <?php if(isset($trades) && $trades->count()): ?>
                    ...
                    <?php $__currentLoopData = $trades; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $trade): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="list-row">
                            <div class="list-icon <?php echo e($trade->type === 'buy' ? 'green' : ($trade->type === 'reward' ? 'gold' : 'red')); ?>"
                                ...
                                <i
                                    class="fa fa-<?php echo e($trade->type === 'buy' ? 'arrow-down' : ($trade->type === 'reward' ? 'gift' : 'arrow-up')); ?>"></i>
                            </div>
                            <div class="list-body">
                                <div class="title"><?php echo e(ucfirst($trade->type)); ?>

                                    <?php if($trade->type === 'buy'): ?>
                                        <span class="badge-app badge-green" style="font-size:10px;margin-left:4px;">BUY</span>
                                    <?php elseif($trade->type === 'reward'): ?>
                                ...
                                <div class="sub"><?php echo e($trade->coin->name ?? 'SVRS'); ?></div>
                            </div>
                            <div style="text-align:right;">
                                <div style="font-size:14px;font-weight:700;color:var(--white);">
                                    <?php echo e(number_format($trade->quantity, 4)); ?> SVRS
                                </div>
                                <div style="font-size:12px;color:var(--muted);">₹<?php echo e(number_format($trade->price, 2)); ?>/coin</div>
                                <div style="font-size:12px;color:var(--muted);">
                                    Total: ₹<?php echo e(number_format($trade->price * $trade->quantity, 2)); ?>
        ...
        function confirmBuy() {
            ...
            Swal.fire({
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    data: { _token: "<?php echo e(csrf_token()); ?>", type: 'buy', price: currentPrice, quantity: qty },
                    success: function (res) {
                        if (res.status) {
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('member.layout.app-layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\Qubeta\svrs\resources\views/member/coin_chart.blade.php ENDPATH**/ ?>